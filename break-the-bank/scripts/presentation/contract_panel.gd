...
func _build_row(index: int) -> Control:
	...
	for state_name: StringName in [&"normal", &"hover", &"pressed", &"disabled", &"focus"]:
		button.add_theme_stylebox_override(state_name,
				UiSkin.row(true, state_name == &"hover" or state_name == &"pressed"))
	...
	head.add_child(_cell("%d." % (index + 1), 17.0, UiSkin.INK_MUTED))
	var name_cell: Label = _cell(contract.display_name, 17.0, UiSkin.INK)
	name_cell.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	head.add_child(name_cell)
	grid.add_child(head)

	var flavour: Label = _cell(contract.description, 13.0, UiSkin.INK_MUTED)
