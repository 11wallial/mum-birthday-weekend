...
func _proof(verb: StringName) -> bool:
	match verb:
		...
		&"pay_doorman":
			return _seen(EffectBus.Event.DOORMAN_PAID)
		&"buy_chit":
			return _seen(EffectBus.Event.CHIT_BOUGHT)
		&"use_chit":
			return _seen(EffectBus.Event.CHIT_USED)
		_:
			return false
...
func test_this_suite_knows_how_to_prove_every_verb() -> void:
	...
	for verb: StringName in SimEngine.PLAYER_VERBS:
		var known: bool = verb in [
			...
			&"press", &"pay_doorman", &"buy_chit", &"use_chit",
		]
